...
void rpdriver_step(void)
{
  ...
  /* Trigonometry: '<S2>/Trigonometric Function3' incorporates:
   *  Trigonometry: '<S2>/Trigonometric Function4'
   *  Trigonometry: '<S2>/Trigonometric Function5'
   */
  ...
  rtb_ResetEncoders = rpdriver_P.SignalGenerator_Frequency *
    rpdriver_M->Timing.t[0];
  ...
  if (rpdriver_P.ResetEncoders1_CurrentSetting == 1) {
    ...
    if (rpdriver_P.ResetEncoders2_CurrentSetting == 1) {
      ...
      if (rtb_ResetEncoders - floor(rtb_ResetEncoders) >= 0.5) {
        /* SignalGenerator: '<Root>/Signal Generator' */
        rtb_ResetEncoders = rpdriver_P.SignalGenerator_Amplitude;
      } else {
        /* SignalGenerator: '<Root>/Signal Generator' */
        rtb_ResetEncoders = -rpdriver_P.SignalGenerator_Amplitude;
      }

      rtb_ResetEncoders *= rpdriver_P.Gain1_Gain;
    }
...
void rpdriver_initialize(void)
{
  ...
  /* External mode info */
  rpdriver_M->Sizes.checksums[0] = (1210005306U);
  rpdriver_M->Sizes.checksums[1] = (3872426070U);
  rpdriver_M->Sizes.checksums[2] = (3051181919U);
  rpdriver_M->Sizes.checksums[3] = (489523426U);
